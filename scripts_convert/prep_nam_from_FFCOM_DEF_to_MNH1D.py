...
"""
Created on August 2021 adapted from prep_nam_from_FFCOM_DEF_to_MNHLES.py
A lancer par python  prep_nam_from_FFCOM_DEF_to_MNH1D.py 
...
g.write(' LBOUSS=.FALSE., CEQNSYS="DUR", LPERTURB=.FALSE.,'+'\n')
...
if (attributes['surface_type'] == 'land' or attributes['surface_type']=='landice') and attributes['surface_forcing_temp'] == 'ts':
  ...
  g.write('&NAM_DATA_TSZ0  NTIME=25 \n')
# ATttention dans SURFEX NTIME=1 ou 25 (nb de subdivisions  dans 1 jour
  if (ntsst <=24):
    for it in range(0,ntsst-1):
          g.write('XUNIF_DTS('+str(it+1)+')='+str(float(sst[it+1]-sst[it])) +',\n')
          print('dt=',str(float(sst[it+1]-sst[it])))
    for it in range(ntsst,26):
          g.write('XUNIF_DTS('+str(it)+')=0. ,\n')
  else:
    ...
    for ik in range(0,nknew):
        if ((attributes['adv_theta']==1) and (attributes['forc_geo']==1)):
            ...
            if (time_ug.shape[0]==time_theta_adv.shape[0]):
                g.write('%.1f %.2f %.2f %.1f %.5f %.5f %.7f %.10f %.1f %.1f' % (lev_theta_adv[ik],utemp[it][ik],vtemp[it][ik],thtemp[it][ik],qtemp[it][ik], wtemp[it][ik],tadvtemp[it][ik],qadvtemp[it][ik],0.,0.)+'\n')
            else:
                g.write('%.1f %.2f %.2f %.1f %.5f %.5f %.7f %.10f %.1f %.1f' % (lev_theta_adv[ik],utemp[0][ik],vtemp[0][ik],thtemp[it][ik],qtemp[it][ik], wtemp[it][ik],tadvtemp[it][ik],qadvtemp[it][ik],0.,0.)+'\n')
        elif ((attributes['adv_thetal']==1) and (attributes['forc_geo']==1)):
            if (ik==0):
                print('size temp',utemp.shape,tadvtemp.shape)
            if (time_ug.shape[0]==time_thetal_adv.shape[0]):
                g.write('%.1f %.2f %.2f %.1f %.5f %.5f %.7f %.10f %.1f %.1f' % (lev_thetal_adv[ik],utemp[it][ik],vtemp[it][ik],thtemp[it][ik],qtemp[it][ik], wtemp[it][ik],tadvtemp[it][ik],qadvtemp[it][ik],0.,0.)+'\n')
            else:
                g.write('%.1f %.2f %.2f %.1f %.5f %.5f %.7f %.10f %.1f %.1f' % (lev_thetal_adv[ik],utemp[0][ik],vtemp[0][ik],thtemp[it][ik],qtemp[it][ik], wtemp[it][ik],tadvtemp[it][ik],qadvtemp[it][ik],0.,0.)+'\n')

        elif (attributes['forc_geo']==1):
            if (ik==0):
                print('size temp',utemp.shape,tadvtemp.shape)
            g.write('%.1f %.2f %.2f %.1f %.5f %.5f %.7f %.10f %.1f %.1f' % (lev_ug[ik],utemp[it][ik],vtemp[it][ik],thtemp[it][ik],qtemp[it][ik], wtemp[it][ik],tadvtemp[it][ik],qadvtemp[it][ik],0.,0.)+'\n')
...
g.write('&NAM_DYNn XTSTEP=60. /\n')
# besoin de modifier pour avoir la bonne durée
g.write('&NAM_DYN XSEGLEN='+str(duration)+', LCORIO=T,\n'	 	)
g.write('             LNUMDIFU=F,\n'   	)
g.write('             LNUMDIFTH = F,\n')
...
if ((casname == 'AMMA') or (casname == 'LBA')):
    g.write('CCLOUD="'+CLOUDSC+'", CSCONV="EDKF",CDCONV="KAFR"  /\n')
else:
    g.write('CCLOUD="'+CLOUDSC+'", CSCONV="EDKF",CDCONV="NONE"  /\n')
...
g.write('&NAM_TURBn XIMPL=1., CTURBLEN="BL89", CTURBDIM="1DIM",\n')
if (CLOUDSC=='KHKO'):
    g.write('           LTURB_FLX=T, LTURB_DIAG=T, LSUBG_COND=F,\n')
else:
    g.write('           LTURB_FLX=T, LTURB_DIAG=T, LSUBG_COND=T,\n')

g.write('           XKEMIN=1E-10,\n')
g.write('           LSIGMAS=T, LSIG_CONV=F, LRMC01=T, CSUBG_AUCV="CLFR" /\n')
g.write('&NAM_PARAM_MFSHALLn CMF_UPDRAFT="EDKF", CMF_CLOUD="DIRE", \n')
g.write('           LMIXUV=T, LMF_FLX=T /\n')
if (CLOUDSC=='KHKO'):
